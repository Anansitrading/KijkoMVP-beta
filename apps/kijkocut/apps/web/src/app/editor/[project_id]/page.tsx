  ...
  const checkApiKey = useCallback(async () => {
    if (window.aistudio && typeof window.aistudio.hasSelectedApiKey === 'function') {
      ...
      if (!selected) {
        ...
        // If not in AI Studio, assume key is available via environment variables
        setHasSelectedApiKey(true);
    }
  }, []);

  useEffect(() => {
    // Check for API key on mount for video features.
    // This is a simplified check; a more robust implementation
    // would check this before any Veo-related API call.
